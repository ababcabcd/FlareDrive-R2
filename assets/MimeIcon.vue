...
// R2 可能把媒体文件存成 application/octet-stream，列表图标需要根据扩展名兜底
const effectiveType = computed(() => {
  const ct = props.contentType || '';
  if (/^(image\/|video\/|audio\/|application\/ogg|application\/pdf|application\/zip|application\/gzip|application\/vnd\.rar)/.test(ct)) {
    return ct;
  }
...
<template>
  <div class="file-icon">
    <img
      v-if="thumbnail && effectiveType.startsWith('image/') && effectiveType !== 'image/svg+xml'"
      :src="thumbnail"
      :width="size"
      :height="size"
      alt="Image"
    ...
    >
      ...
      />
      <path
        v-else-if="effectiveType === 'application/pdf'"
      ...
      />
      <path
        v-else-if="effectiveType.startsWith('video/')"
      ...
      />
      <path
        v-else-if="effectiveType.startsWith('audio/')"
